...
int main(){
    ...
    string temp = "";
    for (int j = 0; j < 0+y.size(); j++){
        temp += x[j];
    }
    ll hash1 = h(b1, m1, temp);

    string temp2 = "";
    for (int j = 0; j < 0+z.size(); j++){
        temp2 += x[j];
    }
    ll hash2 = h(b2, m2, temp2);
    ll mainHash = hash1;
    ll sz = y.size();
    hashMap[combine(hash1, hash2, m3)].insert(mainHash);
    for (int j = 0+z.size(); j < x.size(); j++){
        hash2 = roll(inv2, b2, m2, hash2, x[j], x[j-z.size()], z.size());
        mainHash = add(inv3, b3, m3, mainHash, x[j], sz);
        sz++;
        hashMap[combine(hash1, hash2, m3)].insert(mainHash);
    }
    for (int i = 1; i <  x.size()-z.size()+1; i++){
        hash1 = roll(inv1, b1, m1, hash1, x[i-1], x[i-1+y.size()], y.size());
        sz = y.size();
        hash2 = h(b2, m2, temp2);
        hashMap[combine(hash1, hash2, m3)].insert(mainHash);

        for (int j = i+z.size(); j < x.size(); j++){
            hash2 = roll(inv2, b2, m2, hash2, x[j], x[j-z.size()], z.size());
            mainHash = add(inv3, b3, m3, mainHash, x[j], sz);
            sz++;
            hashMap[combine(hash1, hash2, m3)].insert(mainHash);
    ...
    cout << hashMap[combine(hash1, hash2, m3)].size();
}