...
ld dist(ll x1, ll y1, ll x2, ll y2) {
    return sqrt(pow(x1-x2, 2ll) + pow(y1-y2, 2ll));
}

bool can(ld M) {
    vector<vector<ld>> dp(1<<n, vector<ld>(16, 1000000000.0));
    ...
    for (int msk = 1; msk < (1<<n); msk++) {
        ...
        if (len == 1) {
            ...
            ld time_with_speed = dist(0, 0, x[msb], y[msb])/M;
            ...
            for (int i : all) {
                ...
                for (int j : all) {
                    ...
                    if (dp[prev][j] == 1000000000.0) continue;
    ...
    bool ans = false;
    for (int i = 0; i < n; i++) {
        if (dp[(1<<n)-1][i] != 1000000000.0) ans = true;
    }
...
bool can_brute(ld M, vector<int> &perm) {
    ...
    if (total_time > s[0]) return false;
    for (int i = 1; i < n; i++) {
        int cur = perm[i];
        int prev = perm[i-1];
        total_time += dists[cur][prev]/(M*slows[i]);
        if (total_time > s[cur]) return false;
    }
    return true;
}
...
ld bsearch_brute(ld L, ld R, vector<int> &perm) {
    int cnt = 0;
    while (R-L > 0.0000001) {
        ld M = (R-L)/2 + L;
        cnt++;
...
int main(){
    ...
    while (true) {
        ...
        ld mn = 10000000.0;
        do {
            ld cur_ans = bsearch_brute(0, 10000000.0, perm);
            mn = min(mn, cur_ans);
        ...
        if (abs(ans-mn) >= 0.001) {
            ...
            cout << ".75" << endl;
            cout << setprecision(12) << ans << " " << mn << endl;
            return 0;
        }
    }
}